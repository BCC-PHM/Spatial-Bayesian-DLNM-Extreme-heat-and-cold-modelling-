...
figure1 = ggplot(ward_map %>% left_join(RR_99th_map, by = c("Ward_Code" = "Ward_code" )))+
...
figure2 = ggplot(ward_map %>% left_join(RR_1st_map, by = c("Ward_Code" = "Ward_code" )))+
  geom_sf_interactive(color = "white", aes(fill = RR_median, tooltip = tooltip,data_id = Ward_Code))+
...
Ward-level posterior median annual cold- and heat-related excess mortality rates in Birmingham (MMT-referenced), per 100,000
:::
...
Ward-level posterior median annual excess mortality rates in Birmingham due to temperatures below 2.5th and above 97.5th percentile , per 100,000
:::
:::
...
![Figure 5. Ward-level posterior median projections of annual cold-related excess mortality (%) in Birmingham by emissions scenario (RCP2.6, RCP4.5, RCP6.0, RCP8.5) and decade (baseline, 2030s–2070s)](images/cold_projection_posterior_median_AF.png)

![Figure 6. Ward-level posterior median projections of annual heat-related excess mortality (%) in Birmingham by emissions scenario (RCP2.6, RCP4.5, RCP6.0, RCP8.5) and decade (baseline, 2030s–2070s)](images/heat_projection_posterior_median_AF.png)
