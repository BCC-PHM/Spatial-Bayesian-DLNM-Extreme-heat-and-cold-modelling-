...
Uncertainty was quantified using posterior median and 95% credible interval (CrI) obtained from the posterior distributions. Bayesain inference was addressed by calculating the exceedance probability, where thresholds of 0.95 and 0.9 were used to indicate strong evidence and some evidence respectively. All data processing was performed in R version 4.4.2. All statistical analyses were performed in R version 4.4.2. All data visualisations were performed in R version 4.4.2. The code is available on ....
...
Figure 4 presents the exceedance probability of ward RR relative to the Birmingham mean RR, evaluated at each temperature percentile. The result suggest that there are some evidence that Billesley and Hall Green North are mostly affected by cold exposure, whereas Glebe Farm & Tile Cross shows some evidence in higher exceedance probability at upper temperature percentiles, suggesting some evidence in heat-related risk.
...
#| fig-height: 13  
#| fig-width: 11  
exceedance_prob_df = read_rds(here("output", "Exceedance_prob_RR_greater_mean.rds"))

exceedance_prob_df = exceedance_prob_df %>%
  mutate(tile_id = row_number(),
         evidence = case_when(is.na(p_exceed_bham)  ~ "Missing",
                                 p_exceed_bham >= 0.95 ~ "Strong evidence (≥0.95)",
                                 p_exceed_bham >= 0.90 ~ "Some evidence (0.90–0.95)",
                                 TRUE                 ~ "No evidence (<0.90)"),
...
Across Birmingham, the overall annual excess mortality attributable to cold exposure is 1,902 (95% CrI: 1452-2404) and the overall annual excess mortality attributable to heat exposure is 177 (95% CrI: 87-293). Figure 5 shows the posterior median annual cold and heat-related excess mortality per 100,000 population and the exceedance probability of excess mortality above the overall mean excess mortality. It shows similar pattern where areas with higher excess mortality also have higher RR being reported in figure 2. The exceedance probability suggests that Billesley, Hall Green North and Bromford & Hodgehill have strong evidence that their excess mortality attributable to cold exposure is higher than the overall excess mortality. However, there are no wards that exhibit strong evidence for heat exposure while Glebe Farm & Tile Cross which shows some evidence in elevated RR previously is marginally below the 0.9 threshold for excess mortality attributable to heat.

Across Birmingham, the overall annual excess mortality attributable to percentile temperature below 2.5th is 74 (95% CrI: 59-88) and the overall annual excess mortality attributable to percentile temperature above 97.5th is 29 (95% CrI: 21-38). Figure 6 shows the annual excess mortality per 100,000 for temperatures below 2.5th and above 97.5th percentiles, it demonstrates completely different pattern especially for hotter temperatures where elevated excess mortality is observed in Bilesley and show strong evidence that the exceedance probability is above 0.95 threshold. There are also neighbouring wards around Bilesley showing some evidence in excess mortality. For colder temperatures, the pattern is similar like the one in figure 5 and Bilesley also shows strong evidence in excess mortality in colder exposure and the neighbouring wards around Bilesley show some evidence in excess mortality in colder exposure.

```{r data for figure 5}
#| output: false
#| warning: false
ward_EM_summary = read_rds(here("output", "heat_and_cold_related_EM_ward.rds"))
...
exceed_prob_EM_gt_bham_heatcold = read_rds(here("output", "exceed_prob_EM_gt_bham_heatcold.rds"))
...
ggplot(em_exceedance_Xplot_df )+
  geom_sf_interactive(aes(fill = p_Xcold_gt_bham, tooltip = tooltip_cold, data_id = Ward_Code))+
  scale_fill_distiller(
    palette  = "Blues",
    direction = 1,
    na.value = "grey85",
...
Figure 7 and 8 show the cold- and heat-related excess mortality at ward-level projected under 4 different RCPs . The colour is getting lighter for cold-related excess mortality and conversely getting darker for heat-related excess mortality. The projected change under RCP 8.5 shows the strongest shifts with clearer decline in cold-related excess mortality and heat-related excess mortality by the 2060s and 2070s. Under lower emission pathways (RCP2.6-6.0), changes are less obvious and smaller. Since the projection used the fitted coefficients from the model, the spatial pattern remains similar to previous figures. Overall, these figures illustrate a transition from cold-dominated burden toward increasing heat-related risk later in the century.

![Figure 7. Ward-level posterior median projections of annual cold-related excess mortality (%) in Birmingham by emissions scenario (RCP2.6, RCP4.5, RCP6.0, RCP8.5) and decade (baseline, 2030s–2070s)](images/cold_projection_posterior_median_AF.png)

![Figure 8. Ward-level posterior median projections of annual heat-related excess mortality (%) in Birmingham by emissions scenario (RCP2.6, RCP4.5, RCP6.0, RCP8.5) and decade (baseline, 2030s–2070s)](images/heat_projection_posterior_median_AF.png)
...
The results are broadly comparable with previous studies focusing on nation-wide temperature-mortality association. Almost all of the wards in Birmingham were estimated with MMT(s) that are very close to the range of "comfortable" temperatures that suggested and studied in UK. [@gasparrini2022smallarea ; @ONS2023_climate_mortality_1988_2022]. However, small numbers of wards exhibited lower median MMT(s) which can lead to extremely high heat-related excess mortality. This is because any temperature above median MMT are classified as heat-related and therefore wards with lower MMT(s) will classify a larger proportion of days as "heat", increasing the heat-attributable deaths. This problem has been highlighted in previous UK work, and these results should be interpreted cautiously [@gasparrini2022smallarea]. Consequently, the plot that showed excess mortality below temperatures at 2.5th and above 97.5th percentile displayed completely different patterns. It has to be noted that under the UK climate, the 2.5th and 97.5th percentile of daily mean temperatures correspond to relatively common hotter and colder days. rather than extreme conditions. Overall, the findings indicate the MMT in Birmingham lies at a higher percentile of the observed temperature, implying that large proportion of days fall below the MMT and count as cold-related burden. Therefore, public health policy should focus on preventing the deaths under non-optimal cold temperatures which still dominate the absolute count of burden.

The projection is in line with previous studies where decreasing cold-related mortality and increasing heat-related mortality are estimated for the future [@MURAGE2024; @Gasparrini2017_projections_temp_excess_mortality; @Jenkins_2022]. Under the most extreme RCP 8.5 scenario, the shift is most pronounced where warming is greatest and the divergence between cold- and heat-related excess mortality becomes clearer. As mentioned previously, the projected results did not consider adaptation of population change and therefore results should not be interpreted as predictions. If adaptation were considered, the projected result might draw different conclusion. Recent European studies that incorporate population change suggest that cold-related excess morality can increase before the first half of the century despite warming, it is because aging population increases the pool of vulnerable individuals and increases baseline mortality in older age groups [@GARCIALEON2024e644]. Given that the subnational population projection for Birmingham also indicate aging population, cold-related excess mortality in Birmingham might plausibly increase in the near future even if temperature is getting warmer.

This study has some limitations and the audiences must interpret with caution. Firstly, the estimated CrI(s) were extremely wide for both the observed-period estimates and the projected results, reflecting uncertainty in temperature-mortality relationship with limited data. Fortunately with the use of SB-DLNM, posterior medians are comparatively stable. As a result, the estimates should be interpreted as plausible ranges and broad patterns which are useful for identifying spatial heterogeneity rather than precise estimates for individual wards. Secondly, this study did not use directly age-standardisation but instead the unadjusted crude rate which might underestimate or overestimate the excess mortality rate. The choice was driven by the following two constraints. Ward level population projection are not available and the further separate estimation of temperature-mortality relationship for each age group would substantially increase data sparsity, leading to more unstable estimates. Thirdly, projection with no adaptation held the baseline mortality constant, this isolate purely the impact of climate change but less meaningful to guide immediate policy making given that the population in Birmingham is likely to age. Finally, this study only used 4 out of 12 ensemble members of the UK climate projection which might not capture parts of the internal variability and might not represent the full range of plausible future climates.
