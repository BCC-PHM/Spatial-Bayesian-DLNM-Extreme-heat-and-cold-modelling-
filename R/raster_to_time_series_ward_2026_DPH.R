...
#------------------------------------------------------
tmin_file = sort(list.files(path = "data/raw/2026_tasmin/", full.names = TRUE, pattern = ".nc"))
tmax_file = sort(list.files(path = "data/raw/2026_tasmax/", full.names = TRUE, pattern = ".nc"))
#------------------------------------------------------
#use first raster to set CRS + bham reprojection ONCE
#(assuming all files same CRS)
#------------------------------------------------------
...
output = foreach(
  ...
  #dates
  #--------------------------------------------
  date_data = data.frame(layer_id = seq_along(time(tmin)), date = as.Date(time(tmin)))
  #--------------------------------------------
  #crop + mask
  #--------------------------------------------
  ...
  #extract
  #--------------------------------------------
  tmin_ex = terra::extract(tmin_bham, bham_r, fun = mean, na.rm = TRUE, exact = TRUE)
  tmax_ex = terra::extract(tmax_bham, bham_r, fun = mean, na.rm = TRUE, exact = TRUE)
  #--------------------------------------------
  #long tasmin
  #--------------------------------------------
  ...
  #--------------------------------------------
  out = tasmin_long %>% 
    left_join(tasmax_long, by = c("ID", "Ward_Code", "Ward_Name", "layer_id")) %>% 
...
message(format(Sys.time(), "%H:%M:%S"), " | Processing complete. Total rows: ", nrow(final_output))
...
#save output
#------------------------------------------------------
write_rds(final_output, "data/processed/tmean_ward_daily_2026_DPH.rds")
