...
Across Birmingham, the overall annual excess mortality attributable to cold exposure is 1,458 (95% CrI: 1118-1829) and the overall annual excess mortality attributable to heat exposure is 92 (95% CrI: 63-132). @fig-em-maps shows the posterior median annual cold and heat-related excess mortality per 100,000 population and the exceedance probability of excess mortality above the overall Birmingham median excess mortality (cold: 126.00 per 100,000; heat: 8.00 per 100,000). The exceedance probability suggests that Billesley, Bromford & Hodge Hill and Sutton Walmley & Minworth show strong evidence. Conversely, only Northfield shows very strong evidence, with Glebe Farm & Tile Cross and Bournville & Cotteridge showing strong evidence. Most wards show no clear evidence of a rate above the city median for either exposure.
...
Across Birmingham, the overall annual excess mortality attributable to percentile temperature below 2.5th is 62 (95% CrI: 49-76) and the overall annual excess mortality attributable to percentile temperature above 97.5th is 41 (95% CrI: 35-49). @fig-x-em-maps shows the annual excess mortality per 100,000 for temperatures below 2.5th and above 97.5th percentiles, it demonstrates slightly different pattern especially for hotter temperatures. Yardley East has the highest rate (7.8 per 100,000, 95% CrI 4.3–13.9) and the strongest evidence of being above the city median (Pr = 0.995). Northfield, Sutton Mere Green, Sutton Wylde Green and Castle Vale also show very strong evidence that the excess mortality attributable to hotter temperatures is higher than the overall Birmingham median excess mortality (heat: 3.58 per 100,000). For colder temperatures, Kingstanding (11.8 per 100,000, 95% CrI 6.0–16.3) shows very strong evidence that it is above the Birmingham median (Pr = 0.98). Billesley and Perry Barr also show strong evidence, and several wards in the north and south show some evidence that excess mortality in colder temperatures is higher than the overall Birmingham median excess mortality (cold: 5.37 per 100,000).
...
em_exceedance_plot_df = ward_map %>% 
  ...
  mutate(evidence_cold = case_when(is.na(p_cold_gt_bham)  ~ "Missing",
         ...
         evidence_cold = factor(evidence_cold, levels = evidence_levels),
         evidence_heat = factor(evidence_heat, levels = evidence_levels))
...
figure_EM_prob_heat = ggplot(em_exceedance_plot_df )+
  geom_sf_interactive(aes(fill = evidence_heat, tooltip = tooltip_heat, data_id = Ward_Code),color = "white")+
  scale_fill_manual(
    values = c("Very strong evidence (≥0.95)"="#a50f15", 
               "Strong evidence (0.90–0.95)" ="#ef3b2c", 
               "Some evidence (0.80–0.90)" = "#fc9272", 
               "No evidence (<0.80)"="grey85")
  )+
...
figure_EM_prob_cold = ggplot(em_exceedance_plot_df )+
  geom_sf_interactive(aes(fill = evidence_cold,tooltip = tooltip_cold, data_id = Ward_Code),color = "white")+
  scale_fill_manual(
    values = c("Very strong evidence (≥0.95)"="#08519c", 
               "Strong evidence (0.90–0.95)" ="#4292c6", 
               "Some evidence (0.80–0.90)" = "#9ecae1", 
               "No evidence (<0.80)"="grey85")
  )+
  labs(fill = "Pr(EM>median EM)") +
...
em_exceedance_Xplot_df = ward_map %>% 
  ...
  mutate(evidence_Xcold = case_when(is.na(p_Xcold_gt_bham)  ~ "Missing",
                                   ...
                                   TRUE                 ~ "No evidence (<0.80)"),
         evidence_Xheat = case_when(is.na(p_Xheat_gt_bham)  ~ "Missing",
         ...
         tooltip_cold = paste0(
           ...
           "<B>Inference:</B> ",evidence_Xcold),
         tooltip_heat = paste0(
           "<B>Ward Name:</B> ",Ward_Name, "\n",
           "<B>Ward Code:</B> ", Ward_Code, "\n",
           "<B>Pr(EM > mean EM):</B> ", p_Xheat_gt_bham, "\n",
           "<B>Inference:</B> ",evidence_Xheat),
...
figure_EM_prob_Xheat = ggplot(em_exceedance_Xplot_df )+
  geom_sf_interactive(aes(fill = evidence_Xheat, tooltip = tooltip_heat, data_id = Ward_Code), color = "white")+
 scale_fill_manual(
    values = c("Very strong evidence (≥0.95)"="#a50f15", 
               "Strong evidence (0.90–0.95)" ="#ef3b2c", 
               "Some evidence (0.80–0.90)" = "#fc9272", 
...
ggplot(em_exceedance_Xplot_df )+
  geom_sf_interactive(aes(fill = evidence_Xcold, tooltip = tooltip_cold, data_id = Ward_Code),color = "white")+
   scale_fill_manual(
    values = c("Very strong evidence (≥0.95)"="#08519c", 
               "Strong evidence (0.90–0.95)" ="#4292c6", 
               "Some evidence (0.80–0.90)" = "#9ecae1", 
