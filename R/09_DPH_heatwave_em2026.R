...
#------------------------------------------------------
parts_dir = "output/07_cumlogRR_parts_2026_heatonly_newMMT/"
...
#------------------------------------------------------
baseline_mort = read_rds("output/08_baseline_mort_ward.rds") %>% 
  transmute(
...
june_heat_plot = daily_ts %>% 
  mutate(
    period = case_when(
      Date >= as.Date("2026-06-18") & Date <= as.Date("2026-06-30") ~ "June 2026 heatwave",
      TRUE                                                          ~ NA_character_
    ),
...
ggsave("figs/DPH_2026/09_june_heat_plot.png",
       june_heat_plot  ,
...
july_heat_plot = daily_ts %>% 
  mutate(
    period = case_when(
      Date >= as.Date("2026-07-03") & Date <= as.Date("2026-07-17") ~ "July heatwave",
      TRUE                                                          ~ NA_character_
    ),
    sufficient_evidence = if_else(Pr_positive >= 0.95, "Sufficient evidence", "Insufficient evidence")
  ) %>% 
  ...
  scale_x_date(date_breaks = "1 day", date_labels = "%d %b") +
  labs(
    title    = "Estimated number of heat-attributable deaths during the \nJuly 2026 heatwave in Birmingham",
    x        = "Date",
    y        = "Heat-attributable deaths",
    colour   = NULL
  ) +
...
ggsave("figs/DPH_2026/09_july_heat_plot.png",
       july_heat_plot  ,
       width = pixel_2_in(707,488)[1], height = pixel_2_in(707,488)[2],
       units = "in",
       dpi=600)
